...
function updateDistribution() {
  ...
  // Actualizar los porcentajes mostrados (badges)
  const basicBadge = document.getElementById("basic-needs-badge");
  const personalBadge = document.getElementById("personal-badge");
  const savingsBadge = document.getElementById("savings-badge");

  if (basicBadge) basicBadge.textContent = `${distributionConfig.basicNeeds}%`;
  if (personalBadge) personalBadge.textContent = `${distributionConfig.personal}%`;
  if (savingsBadge) savingsBadge.textContent = `${distributionConfig.savings}%`;
}
